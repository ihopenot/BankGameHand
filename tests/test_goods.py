...
class TestGoodsType:
    def test_attributes(self):
        gt = GoodsType(name="芯片", base_price=5000, bonus_ceiling=0.8)
        assert gt.name == "芯片"
        assert gt.base_price == 5000
        assert gt.bonus_ceiling == 0.8

    def test_type_annotations(self):
        gt = GoodsType(name="硅", base_price=1000, bonus_ceiling=1.0)
        assert isinstance(gt.base_price, int)  # Money
        assert isinstance(gt.bonus_ceiling, float)  # Radio
